   ...
   public class HttpClient
    {
        public async Task<T> GetJSON<T>(string path, object queryParams = null, object headers = null, object cookies = null)
        {
            try
            {
                return await new Url(path)
                    ...
                    .WithTimeout(FlurlHttpConfig.REST_REQUEST_TIMEOUT)
        ...
        public async Task<T> PostJSONAsync<T>(string path, object payload = null,
            int timeout = FlurlHttpConfig.REST_REQUEST_TIMEOUT,
                                                  ...
                                                  object cookies = null)
        {
            try
            {
                return await new Url(path).WithCookies(cookies ?? new { }).WithTimeout(FlurlHttpConfig.REST_REQUEST_TIMEOUT)
                                  .WithHeaders(headers ?? new { })
            ...
            try
            {
                using (Stream primaryFileStream = new MemoryStream(imageBytes))
                {
                    using (Stream secondaryFileStream = secondaryImageBytes == null ? new MemoryStream() : new MemoryStream(secondaryImageBytes))
                        return await new Url(path).WithTimeout(FlurlHttpConfig.REST_REQUEST_TIMEOUT).PostMultipartAsync((mp) =>
                        {
                                                  ...
                                                  object cookies = null)
        {
            try
            {
                await new Url(path).WithCookies(cookies ?? new { }).WithTimeout(FlurlHttpConfig.REST_REQUEST_TIMEOUT)
                                  .WithHeaders(headers ?? new { })
                              ...
                              object cookies = null)
        {
            try
            {
                return await new Url(path)
                         .SetQueryParams(queryParams ?? new { })
                    .WithCookies(cookies ?? new { }).WithTimeout(FlurlHttpConfig.REST_REQUEST_TIMEOUT)
                                   .WithHeaders(headers ?? new { })
                              ...
                              object cookies = null)
        {
            ...
            {
                await new Url(path)
                   .SetQueryParams(queryParams ?? new { }).WithTimeout(FlurlHttpConfig.REST_REQUEST_TIMEOUT)
                   .WithCookies(cookies ?? new { })
                                  .WithHeaders(headers ?? new { })
                                         .DeleteAsync();
            }
